...
public class Source {
	...
	public static class rpn {
		static class NotDFA {		// Skonczony automat deterministyczny [SAD]
			public enum State {		// Stany SAD
				...
				static {
					...
					// Przejscia w zaleznosci o wczytanej wartosci
					// Stan q0
					q0.z = q1;
					q0.lp = q0;
					q0.o1 = q2;
		...
		public static String rpnParse(String infStr) {
			...
			NotDFA.State	autoState		= NotDFA.State.q0;

			// if (inputStr.contains("(r+y)=a=(b+c)+")) {
			// 	System.out.print();
			// }
			...
			for (int i = 0; i < infStr.length(); i++) {
				...
				if (isCorrectINFSymbol(sym)) {
					if (isOperand(sym)) {
						// OP_count++;
						autoState = autoState.readNext("z");

						result.push(sym);
					} else if (sym == '(') {
						autoState = autoState.readNext("(");
						leftPar_count++;
						...
						if (isRightSide(sym)) {	// Napotkany prawostronnie laczny operator
							if (isUnary(sym)) {
								// unary_OR_count++;
								autoState = autoState.readNext("o1");
							} else {
								// binary_OR_count++;
								autoState = autoState.readNext("o2");
							}
							while (stack.NonEmpty() && stack.peek() != '('
					...
					if (autoState.hasError()) { return "Error"; }
				}

			}
			// if (binary_OR_count * 2 + unary_OR_count != OP_count) {
			// 	return "Error";
			// }
				...
				if (isCorrectRPNSymbol(sym)) {
					if (isOperand(sym)) {
						...
						if (isUnary(sym)) {
							...
							if ((isUnary(prevOB) == false) && getPriority(sym) >= getPriority(prevOB)) {
